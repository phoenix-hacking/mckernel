...
base = Path('/work/native-runtime-cpu-exact-numa-20260907')
...
if Path('/work/native-cpu-exact-stage-20260907/build.phase').read_text().strip() != 'complete':
...
(base / 'root/expected-numa-nodes').write_text('0-1\n')
...
inputs += [Path('/work/native-cpu-exact-stage-20260907/kbuild-link-closure.json'),
           repo / 'scripts/tests/fixtures/native-cpu-reservation.c',
           repo / 'scripts/native-cpu-reservation-init.sh',
           repo / 'host-kernel/kbuild/patches/0003-driver-core-export-device-hotplug-transactions.patch',
           Path('/work/write-native-cpio-spec.py'),
           Path('/work/native-runtime-24a151fe/gen_init_cpio'),
           Path('/usr/bin/cc').resolve()]
inputs += [path for path in sorted((source / 'drivers/misc/mckernel').rglob('*')) if path.is_file()]
record = dict(kind='native-cpu-exact-stage-two-numa-nodes', qemu_args=args,
              ...
              probe_compile_commands=probe_commands,
              ...
              staging_lock_refreshed=True, numa_nodes=2, production_gate_credit=False,
...
expected = ['NATIVE_CPU_GUEST CYCLE=' + str(cycle) + ' restored=0-3 unloaded=all PASS'
            for cycle in (1, 2)] + ['NATIVE_CPU_GUEST COMPLETE PASS',
                          'NATIVE_CPU_GUEST NUMA nodes=0-1 cpu-map=0-1/2-3 PASS']
