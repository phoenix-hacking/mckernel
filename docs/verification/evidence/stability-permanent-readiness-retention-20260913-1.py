...
work=Path('/work');out=work/'stability-permanent-readiness-retained-20260913-1';out.mkdir()
assert shutil.disk_usage(work).free>3*1024**3
record=dict(schema_version=1,status='RUNNING',started_utc=datetime.now(timezone.utc).isoformat(),scope='Preserved permanent preparation1 proof-schema failure; successful fresh preparation2/root/preflight; no permanent guest execution or acceptance',artifacts=[],captures=[],new_catalog_acceptance=False,actual_transport_fault_verified=False,production_gate_credit=False)
