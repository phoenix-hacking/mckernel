...
work=Path('/work');out=work/'stability-published-build-collection-retained-20260913-1';out.mkdir()
assert shutil.disk_usage(work).free>3*1024**3
record=dict(schema_version=1,status='RUNNING',started_utc=datetime.now(timezone.utc).isoformat(),scope='Actual version2 native module build and known stack review, Linux collector build/SHA9/nonroot rejection, mode2 root preparation and physical18; all guest acceptance disabled pending cancellation ownership repair',artifacts=[],captures=[],new_catalog_acceptance=False,actual_transport_fault_verified=False,production_gate_credit=False)
...
try:
 ...
 orchestration=work/'stability-published-build-orchestration-20260913-1';orchestration.mkdir()
 orchestration_record=dict(status='SOURCE_EVIDENCE_WITH_PRESERVED_FAILURE',files=[],guest_executed=False)
 for leaf in ('build-published-hold.py','build-published-hold-1.log','build-linux-collector.py','build-linux-collector-before-review.py','build-linux-collector-1.log','prepare-published-guest.py','prepare-published-guest-1.log','check-published-physical.py','check-published-physical.log'):
