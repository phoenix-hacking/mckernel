...
work=Path('/work');out=work/'stability-review-retained-20260913-3';out.mkdir()
...
try:
    ...
    names=['stability-infrastructure-tests-20260913-1','stability-infrastructure-author-evidence-20260913-1']
    for i,name in enumerate(names):
        source=work/name; child=json.loads((source/'record.json').read_text())
        assert child['status']==['PASS','SOURCE_EVIDENCE_WITH_PRESERVED_FAILURE'][i]
        archive=out/('stability-review-20260913-3-'+str(i+1).zfill(2)+'.tar.gz')
