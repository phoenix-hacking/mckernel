...
work=Path('/work');out=work/'stability-terminal-integration-retained-20260913-1';out.mkdir()
assert shutil.disk_usage(work).free>3*1024**3
record=dict(schema_version=1,status='RUNNING',started_utc=datetime.now(timezone.utc).isoformat(),scope='Preserved guest5 hard-fault contract failure and physical-only diagnosis; controller v2 build, actual eight-case Linux boundary checks, root5 preparation and metadata12 infrastructure; no transport or catalog acceptance',artifacts=[],captures=[],new_catalog_acceptance=False,actual_transport_fault_verified=False,production_gate_credit=False)
...
try:
 ...
 names=['stability-transport-fault-run-20260913-prepublish-hard-5', 'stability-transport-fault-guest-20260913-prepublish-hard-5', 'stability-hard-physical-diagnostic-20260913-1', 'stability-owner-terminal-controller-build-20260913-1', 'stability-fault-guest-preparation-20260913-5', 'stability-fault-guest-root-20260913-prepublish-hard-5', 'stability-fault-guest-preflight-20260913-5', 'stability-terminal-boundary-tests-20260913-1', 'stability-application-preflight-tests-20260913-1']
 for i,name in enumerate(names):
  source=work/name;child=json.loads((source/'record.json').read_text());before=identity(source/'record.json')
  assert child['status'] in ('PHYSICAL_COMPARISONS_MATCH_ORIGINAL_RUN_FAIL','PASS_ACTUAL_LINUX_TERMINAL_BOUNDARY_ONLY','PASS_METADATA_PREFLIGHT_TESTS_ONLY','PASS_SYNTHETIC_PHYSICAL_COMPARATOR_ONLY','PASS_ACTUAL_RECEIVER_GATE_INFRASTRUCTURE_ONLY','PASS_BUILD_ONLY','FAIL','PASS','PREPARED_NOT_COMPILED_NOT_EXECUTED','SOURCE_EVIDENCE_WITH_PRESERVED_FAILURE','PASS_BUILD_AND_SYNTHETIC_COLLECTION_TESTS_ONLY','PASS_ACTUAL_C_PTY_INFRASTRUCTURE_ONLY','PASS_SYNTHETIC_FAULT_CONTROL_ONLY','PASS_PREPARED_NO_GUEST_EXECUTION','PREPARED_NOT_EXECUTED','PASS_PREFLIGHT_NO_GUEST_EXECUTION','PASS_SYNTHETIC_PHASE_PARSER_ONLY')
  archive=out/('stability-terminal-integration-20260913-1-'+str(i+1).zfill(2)+'.tar.gz')
