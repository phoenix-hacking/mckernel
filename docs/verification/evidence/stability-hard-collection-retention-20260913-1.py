...
work=Path('/work');out=work/'stability-hard-collection-retained-20260913-1';out.mkdir()
assert shutil.disk_usage(work).free>3*1024**3
record=dict(schema_version=1,status='RUNNING',started_utc=datetime.now(timezone.utc).isoformat(),scope='Guest6 complete native hard-fault collection and physical comparison awaiting independent acceptance; permanent mode module build only; no catalog acceptance',artifacts=[],captures=[],new_catalog_acceptance=False,actual_transport_fault_verified=False,production_gate_credit=False)
...
try:
 ...
 names=['stability-transport-fault-run-20260913-prepublish-hard-6', 'stability-transport-fault-guest-20260913-prepublish-hard-6', 'stability-hard-physical-comparison-20260913-1', 'stability-transport-fault-module-20260913-permanent-backpressure-1']
 for i,name in enumerate(names):
  ...
  archive=out/('stability-hard-collection-20260913-1-'+str(i+1).zfill(2)+'.tar.gz')
