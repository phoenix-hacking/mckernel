...
work=Path('/work');out=work/'stability-return-poll-collection-retained-20260913-1';out.mkdir()
assert shutil.disk_usage(work).free>3*1024**3
record=dict(schema_version=1,status='RUNNING',started_utc=datetime.now(timezone.utc).isoformat(),scope='Actual C sample classification21 only; original controller polling false observation remains retained; no full observe_return state-machine or guest execution',artifacts=[],captures=[],new_catalog_acceptance=False,actual_transport_fault_verified=False,production_gate_credit=False)
