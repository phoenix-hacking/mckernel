...
out = work / 'stability-transport-fault-run-20260913-permanent-backpressure-1'
...
record = dict(status='RUNNING', started_utc=datetime.now(timezone.utc).isoformat(), inputs=[],
              ...
              new_catalog_payloads_executed=0, mode='permanent-backpressure')
def identity(path):
...
try:
    ...
    hashes = {
        ...
        'scripts/tests/prepare_stability_fault_guest.py': 'e64dc3cedfef6877fc47e3ee4ce53f85f981a0095cd3b74f2438d43980ca4d98',
    ...
    for name, status in (
            ('stability-owner-terminal-controller-build-20260913-1', 'PASS_BUILD_ONLY'),
            ('stability-terminal-boundary-tests-20260913-1', 'PASS_ACTUAL_LINUX_TERMINAL_BOUNDARY_ONLY'),
            ...
            ('stability-fault-guest-root-20260913-permanent-backpressure-1', 'PREPARED_NOT_EXECUTED'),
            ('stability-fault-guest-preflight-20260913-permanent-1', 'PASS_PREFLIGHT_NO_GUEST_EXECUTION'),
    ...
    assert json.loads((work / 'stability-fault-guest-root-20260913-permanent-backpressure-1/record.json').read_text())['payload_profile'] == 'runnable-thread-v1'
    ...
    built_controller = json.loads((work / 'stability-owner-terminal-controller-build-20260913-1/record.json').read_text())
    assert built_controller['controller_profile'] == 'owner-phase-v2'
    assert built_controller['controller_source']['sha256'] == '7b1115f4c744d32bb085f23b515e35bcfab4cf0efa6f2e3cb5827742876511e2'
    assert json.loads((work / 'stability-fault-guest-root-20260913-permanent-backpressure-1/record.json').read_text())['controller_profile'] == 'owner-phase-v2'
    for row in built_controller['compiler_dependencies'] + built_controller['compiled_outputs'] + built_controller['loader_dependencies']:
        assert identity(Path(row['path'])) == row
    ...
    compiled = work / 'stability-transport-fault-module-20260913-permanent-backpressure-1/record.json'
    ...
    stack_path = repo / 'docs/verification/stability-permanent-stack-review-20260913-1.json'
    stack = json.loads(stack_path.read_text())
    assert stack['status'] == 'KNOWN_MODULE_FRAME_PATHS_NOT_INCREASED_WITH_EXPLICIT_KERNEL_RESIDUALS'
    assert stack['build_record']['sha256'] == identity(compiled)['sha256']
    target = out / 'permanent-stack-review.json'; shutil.copyfile(stack_path, target)
    record['inputs'].append(dict(original=identity(stack_path), retained=identity(target)))
    assert module['mode'] == 'permanent-backpressure' and module['shared_production_trees_restored']
    ...
    guest = work / 'stability-transport-fault-guest-20260913-permanent-backpressure-1'
    argv = ['/usr/bin/python3', '-B', str(out / 'scripts/tests/run_stability_transport_guest.py'),
            '--prepared', '/work/stability-fault-guest-root-20260913-permanent-backpressure-1', '--output', str(guest)]
