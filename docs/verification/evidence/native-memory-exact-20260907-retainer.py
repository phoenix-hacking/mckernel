...
build=scratch/'native-memory-exact-stage-20260907'; runtime=scratch/'native-runtime-memory-exact-stage-20260907'
...
def put(label,data,compressed=False):
 target=out/('native-memory-exact-20260907-'+label)
...
value=dict(schema_version=1,kind='native-memory-exact-stage-debug-checkpoint',recorded_utc=datetime.now(timezone.utc).isoformat(),source_parent=b['source_parent'],build=b,runtime_result={k:r[k] for k in ['status','qemu_exit_code','started_utc','finished_utc','cpus','memory_mib','numa_nodes','staging_lock_refreshed','memory_reservation_proven','memory_assignment_proven','mckernel_boot_proven','production_gate_credit']},scope='declared stage with isolated fault-injection debug configuration; both ABIs across two module cycles',full_repository_suite_passed=False,remaining=['runtime/workflow and current license verification bindings','fresh full repository suite','OS resource assignment, image loading and AP startup','IKC and applications','McKernel Rust/assembly-only completion and production acceptance'],artifacts=artifacts)
p=repo/'docs/verification/native-memory-exact-stage-checkpoint-20260907.json'
