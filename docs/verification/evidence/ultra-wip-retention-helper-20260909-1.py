...
record=dict(status='RUNNING',started_utc=datetime.now(timezone.utc).isoformat(),application_handoff_ready=False,scope='WIP review/source checkpoint; controlled protocol results only, no new module/image/runtime acceptance',artifacts=[],captures=[])
def identity(p):
 data=p.read_bytes();return dict(size=len(data),sha256=hashlib.sha256(data).hexdigest())
try:
 for n,expected in [('native-application-transport-failure-20260909-1','FAIL'),('native-application-transport-failure-20260909-2','PASS')]:
  ...
  with tarfile.open(dest,'r:gz') as tar:
   for member in tar.getmembers():
    ...
    if member.isdir(): assert current.is_dir()
     ...
     assert member.isfile() and current.is_file(),member.name
 ...
 helper=out/'ultra-wip-retention-helper-20260909-1.py';shutil.copyfile(__file__,helper)
 ...
 record['finished_utc']=datetime.now(timezone.utc).isoformat();(out/'ultra-wip-checkpoint-20260909-1.json').write_text(json.dumps(record,indent=2,sort_keys=True)+'\n');print(record['status'],out,flush=True)
