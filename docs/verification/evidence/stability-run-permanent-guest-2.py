...
out = work / 'stability-transport-fault-run-20260913-permanent-backpressure-2'
...
try:
    ...
    for name, status in (
            ...
            ('stability-fault-guest-root-20260913-permanent-backpressure-2', 'PREPARED_NOT_EXECUTED'),
            ('stability-fault-guest-preflight-20260913-permanent-2', 'PASS_PREFLIGHT_NO_GUEST_EXECUTION'),
    ...
    assert json.loads((work / 'stability-fault-guest-root-20260913-permanent-backpressure-2/record.json').read_text())['payload_profile'] == 'runnable-thread-v1'
    ...
    assert json.loads((work / 'stability-fault-guest-root-20260913-permanent-backpressure-2/record.json').read_text())['controller_profile'] == 'owner-phase-v2'
    ...
    argv = ['/usr/bin/python3', '-B', str(out / 'scripts/tests/run_stability_transport_guest.py'),
            '--prepared', '/work/stability-fault-guest-root-20260913-permanent-backpressure-2', '--output', str(guest)]
