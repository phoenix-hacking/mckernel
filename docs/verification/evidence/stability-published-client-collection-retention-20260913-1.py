...
work=Path('/work');out=work/'stability-published-client-collection-retained-20260913-1';out.mkdir()
assert shutil.disk_usage(work).free>3*1024**3
record=dict(schema_version=1,status='RUNNING',started_utc=datetime.now(timezone.utc).isoformat(),scope='Actual published controller build and C metadata81 only; no live client ioctl/controller/guest execution; complete405 per-case artifacts',artifacts=[],captures=[],new_catalog_acceptance=False,actual_transport_fault_verified=False,production_gate_credit=False)
