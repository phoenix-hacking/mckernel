...
repo = Path('/workspace')
out = Path('/work/rust-consumers-trampoline-region-20260907.json')
...
for path in paths:
    ...
    elif path == 'scripts/tests/fixtures/mckernel_trampoline_region_verify.rs':
        disposition = 'verification_fixture'
        build = ['scripts/native-trampoline-region-init.sh', 'docs/verification/native-trampoline-region-checkpoint-20260907.json']
        pending = 'Reuse the tested low-memory owner in native SMP boot with exact OS/CPU/IRQ lifetime integration.'
...
record['generator'] = dict(path=__file__, **identity(Path(__file__)))
if out.exists(): raise SystemExit('Refusing to replace prior inventory')
