# Linux collector storage-fault-v2 source correction packet 17

Status: DRAFT FOR INDEPENDENT PACKET REVIEW ONLY

This replaces rejected packet 16 and preserves packets 1 through 11, 13 and
15. It closes only the six independently reproduced gaps in source-review
attempt 12/13. It does not release compilation, root execution, application
acceptance or production credit.
...
## Truthful cleanup chronology and fixed budgets
...
The direct child's governing wait deadline is
`min(original_direct_wait_deadline_ns, supervisor_cleanup_deadline_ns)`. The
original deadline is the owner-wait deadline for a spawned owner and the
sentinel-wait deadline for a failed-preflight sentinel. A direct `wait-timeout`
records the governing deadline actually reached. Thus ordinary owner-wait
expiration retains the original 220-second deadline, while cleanup triggered by
an earlier pump/helper failure truthfully records the earlier terminal cleanup
deadline. It never fabricates a timeout at a future original deadline and never
extends the fixed 22-second cleanup bound.

Each successful direct TERM or KILL attempt first retains its signal event and
uses that event's exact `monotonic_ns` as the budget origin. Its stage deadline
is exactly `min(supervisor_cleanup_deadline_ns, signal_event.monotonic_ns +
1 second)` for TERM or the same expression plus 5 seconds for KILL. A stage
`wait-timeout` has exactly that deadline and a timestamp from that deadline
through `finished_ns`. Shortened, extended or reset stage budgets are rejected.
...
`kill-pre-signal` and `owned-scan`. Every timeout PID/birth joins the direct or
adopted identity applicable at that point.
...
`cleanup_deadline_ns == cleanup_start_ns + 15 seconds`. Pre-fork selectors
retain no fabricated post-fork observations.

For report-present selectors 0, 3 and 5, the report child wait, witness REAP and
actual-reap event raw values are byte-for-byte equal and each is independently
decoded as exit zero/no signal. For selector 4, the oracle independently proves
the report artifact is absent and joins only the witness REAP with the
actual-reap event; both available raw waits receive the same independent
exit-zero/no-signal validation. Report absence never supplies a synthetic third
wait.
...
- a future timeout after `finished_ns` is rejected; truthful deadline-plus-one
  OWNER_ERROR evidence within the overall deadline accepts; and an early
  supervision failure with an unreaped owner round-trips using the cleanup
  deadline rather than the future 220-second deadline;
- TERM and KILL deadline mutations that shorten, extend or reset the fixed
  signal-event-relative 1/5-second budgets are rejected;
...
- each post-fork selector independently rejects nonzero/nonterminal REAP,
  duplicate EOF and altered 15-second cleanup arithmetic, with separate
  report-present and selector-4 report-absent joins;
