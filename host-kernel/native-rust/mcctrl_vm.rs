...
use core::{ffi::{c_char, c_void}, mem::{align_of, size_of, MaybeUninit}, ptr::{self, NonNull}, sync::atomic::{AtomicI32, Ordering}};
...
extern "C" {
    fn anon_inode_getfile(name: *const c_char, operations: *const bindings::file_operations,
                         private: *mut c_void, flags: i32) -> *mut bindings::file;
}
...
impl MmIdentity {
    fn from_current(mm: &CurrentMm) -> Result<Self> {
        ...
        let count = unsafe { AtomicI32::from_ptr(ptr::addr_of_mut!((*mm.0.as_ptr()).__bindgen_anon_1.__bindgen_anon_1.mm_count.counter)) };
        count.fetch_update(Ordering::Relaxed, Ordering::Relaxed, |n|
            (n > 0).then(|| n.checked_add(1)).flatten()).map_err(|_| errno(-75))?;
        Ok(Self(mm.0))
    }
}
impl Drop for MmIdentity {
    fn drop(&mut self) {
        ...
        let count = unsafe { AtomicI32::from_ptr(ptr::addr_of_mut!((*self.0.as_ptr()).__bindgen_anon_1.__bindgen_anon_1.mm_count.counter)) };
...
struct MappingFile { registration: Arc<Registration>, mirror: Arc<Mirror> }
...
impl Mirror {
    pub(super) fn reserve(registration: Arc<Registration>) -> Result<Arc<Self>> {
        ...
        let first = unsafe {
            ...
            let first = if vma.is_null() { None } else { Some((*vma).__bindgen_anon_1.__bindgen_anon_1.vm_start) };
            bindings::up_read(sem);
            first
        };
        let end = wire::reservation_end(first).map_err(errno)?;
        let mirror = Arc::new(Self { mm: MmIdentity::from_current(&mm)?, start: 0, end }, GFP_KERNEL)?;
        let private = Box::new(MappingFile { registration, mirror: mirror.clone() }, GFP_KERNEL)?;
        ...
        let file = unsafe { anon_inode_getfile(c"[mckernel]".as_ptr(), &FILE_OPERATIONS.0, private.cast(), 2) };
        if (-4095..0).contains(&(file as isize)) || file.is_null() {
            ...
            return Err(if file.is_null() { EIO } else { errno(file as isize as i32) });
        }
        let result = (|| -> Result {
            ...
            if promoted.is_null() { return Err(ENOMEM); }
            let address = unsafe {
                ...
                let address = bindings::vm_mmap(file, 0, end as usize, 7, 0x100001, 0);
                bindings::revert_creds(original);
                bindings::abort_creds(promoted);
                address
            };
            if (-4095..0).contains(&(address as isize)) { return Err(errno(address as isize as i32)); }
            if address != 0 { return Err(EIO); }
            Ok(())
        })();
    ...
    pub(super) fn current(&self) -> Result {
        if CurrentMm::get()?.0 != self.mm.0 { return Err(errno(-18)); }
        Ok(())
    }
    ...
    pub(super) fn unmap(&self) -> Result {
        ...
        kernel::error::to_result(unsafe { bindings::vm_munmap(self.start, (self.end - self.start) as usize) }).map(|_| ())
    }

    pub(super) fn clear(&self, start: u64, end: u64) -> Result {
        if start >= end || start < self.start || end > self.end || start % 4096 != 0 || end % 4096 != 0 { return Err(EINVAL); }
        let mm = CurrentMm::get()?;
        if mm.0 != self.mm.0 { return Err(errno(-18)); }
            ...
            let result = (|| -> Result {
                for apply in [false, true] {
                    ...
                    while address < end {
                        ...
                        if vma.is_null() || (*vma).__bindgen_anon_1.__bindgen_anon_1.vm_start > address
                            || (*vma).vm_ops != &VM_OPERATIONS.0 || (*vma).vm_file.is_null() { return Err(EINVAL); }
                        let private = &*(*(*vma).vm_file).private_data.cast::<MappingFile>();
                        if !ptr::eq(&*private.mirror, self) { return Err(EACCES); }
                        let last = end.min((*vma).__bindgen_anon_1.__bindgen_anon_1.vm_end);
                        if apply { bindings::zap_vma_ptes(vma, address, last - address); }
                        address = last;
                    }
                }
                Ok(())
...
unsafe extern "C" fn mmap(file: *mut bindings::file, vma: *mut bindings::vm_area_struct) -> i32 {
    ...
    unsafe {
        if (*vma).vm_mm != mirror.mm.0.as_ptr() || (*vma).__bindgen_anon_1.__bindgen_anon_1.vm_start != mirror.start
            || (*vma).__bindgen_anon_1.__bindgen_anon_1.vm_end != mirror.end || (*vma).vm_pgoff != 0 { return EINVAL.to_errno(); }
        (*vma).__bindgen_anon_2.__vm_flags |= (bindings::VM_PFNMAP | bindings::VM_DONTEXPAND | bindings::VM_DONTDUMP) as u64;
...
unsafe extern "C" fn fault(vmf: *mut bindings::vm_fault) -> bindings::vm_fault_t {
    let result = (|| -> Result<bindings::vm_fault_t> {
        ...
        let (vma, address, flags) = unsafe { ((*vmf).__bindgen_anon_1.vma, (*vmf).__bindgen_anon_1.address, (*vmf).flags) };
        let private = unsafe { &*(*(*vma).vm_file).private_data.cast::<MappingFile>() };
        let mirror = &private.mirror;
        if unsafe { (*vma).vm_mm } != mirror.mm.0.as_ptr() || address < mirror.start || address >= mirror.end { return Err(EACCES); }
        ...
        private.registration.invoke(super::application_abi::LOOKUP, &mut request)?;
        ...
        unsafe {
            ...
            if permissions & 1 == 0 { protection.pgprot &= !2; }
            if permissions & 2 == 0 { protection.pgprot |= 1 << 63; }
            Ok(bindings::vmf_insert_pfn_prot(vma, address & !4095, physical >> 12, protection))
        }
    })();
...
unsafe extern "C" fn mprotect(_: *mut bindings::vm_area_struct, _: u64, _: u64, _: u64) -> i32 { errno(-95).to_errno() }
unsafe extern "C" fn mremap(_: *mut bindings::vm_area_struct) -> i32 { errno(-95).to_errno() }
...
unsafe extern "C" fn release(_: *mut bindings::inode, file: *mut bindings::file) -> i32 {
    let private = unsafe { (*file).private_data.cast::<MappingFile>() };
    unsafe { (*file).private_data = ptr::null_mut(); drop(Box::from_raw(private)); }
    0
}
...
static FILE_OPERATIONS: FileOperations = FileOperations({
    ...
    operations.mmap = Some(mmap); operations.release = Some(release); operations
});
static VM_OPERATIONS: VmOperations = VmOperations({
    // SAFETY: Null optional callbacks are valid; all supplied code is retained.
    let mut operations: bindings::vm_operations_struct = unsafe { MaybeUninit::zeroed().assume_init() };
    operations.fault = Some(fault); operations.pfn_mkwrite = Some(fault);
    operations.mprotect = Some(mprotect); operations.mremap = Some(mremap); operations
});
