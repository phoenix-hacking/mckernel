...
impl Entry {
    fn key(&self) -> Token { self.cleanup.token() }
    ...
    fn request_cleanup(&mut self) -> Result {
        ...
        if self.quarantined { return Err(errno(-71)); }
        if let Some(prepare) = &self.prepare {
            if prepare.result().is_none() { return Ok(()); }
            if self.cleanup.reserved() {
                self.cleanup.cleanup_target(prepare.input.cpu, prepare.thread).map_err(errno)?;
            }
        }
        if self.cleanup.reserved() { self.cleanup.begin().map_err(errno)?; }
        Ok(())
    }
}
...
impl Remote {
    pub(crate) fn new(owner: OsToken) -> Result<Arc<Self>> {
        ...
        for _ in 0..CAPACITY { slots.push(None, GFP_KERNEL)?; }
        Arc::pin_init(pin_init!(Self { owner, slots <- new_mutex!(slots) }), GFP_KERNEL)
    }

    pub(crate) fn reserve(&self, pid: i32) -> Result<Token> {
        let mut slots = self.slots.lock();
        // Late and quarantined requests continue excluding numeric PID reuse.
        if slots.iter().flatten().any(|entry| entry.cleanup.pid() == pid) { return Err(EINVAL); }
        ...
        *slot = Some(Entry { cleanup, prepare: None, needs_cleanup: false, closed: false, quarantined: false });
        Ok(token)
    }

    pub(crate) fn queued_cpu(&self) -> Option<i32> {
        self.slots.lock().iter().flatten().map(Entry::next)
            .find(|exchange| exchange.queued()).map(Exchange::cpu)
    }

    pub(crate) fn publish(&self, cpu: i32, send: impl FnOnce(&[u8; 128]) -> Result) -> Result<bool> {
        let mut slots = self.slots.lock();
        let Some(exchange) = slots.iter_mut().flatten().map(Entry::next_mut)
            .find(|exchange| exchange.queued() && exchange.cpu() == cpu) else { return Ok(false); };
    ...
    pub(crate) fn reply(&self, packet: &[u8; 128], mut memory: impl FnMut(u64, usize) -> Result) -> Result {
        let mut slots = self.slots.lock();
        for slot in &mut *slots {
            let Some(entry) = slot.as_mut() else { continue; };
            if let Some(prepare) = entry.prepare.as_mut().filter(|prepare| prepare.result().is_none()) {
                match prepare.exchange.accept(packet) {
                    Ok(()) => {
                        let outcome = prepare.finish(&mut memory);
                        entry.quarantined = prepare.exchange.result() == Some(0) && outcome.is_err();
                        ...
                        if entry.needs_cleanup && !entry.quarantined { entry.request_cleanup()?; }
                        return Ok(());
                    }
                    Err(-2 | -16) => continue,
    ...
    pub(crate) fn close(&self, token: Token) {
        let mut slots = self.slots.lock();
        if let Some(slot) = slots.iter_mut().find(|slot| slot.as_ref().is_some_and(|entry| entry.key() == token)) {
            ...
            if entry.prepare.is_none() && entry.cleanup.reserved() || entry.cleanup.result().is_some() {
                *slot = None;
            } else {
                entry.closed = true;
                entry.cleanup.abandon();
                if let Some(prepare) = &mut entry.prepare { prepare.exchange.abandon(); }
                let _ = entry.request_cleanup();
            }
        }
    }

    pub(crate) fn prepare(&self, token: Token, bytes: &mut [u8], cpus: usize, direct_map: u64) -> Result {
        ...
        let mut prepare = Preparation::new(self.owner.slot() as i32, bytes, cpus, direct_map)?;
        {
            ...
            if entry.closed || entry.needs_cleanup || entry.prepare.is_some() || !entry.cleanup.reserved() {
                return Err(EBUSY);
            }
            if prepare.input.pid != entry.cleanup.pid() { return Err(EINVAL); }
        ...
        for _ in 0..3000 {
            {
                ...
                if prepare.result().is_some() { return prepare.copy_result(bytes); }
            }
    ...
    pub(crate) fn with_prepared<T>(&self, token: Token, use_image: impl FnOnce(&Preparation) -> Result<T>) -> Result<T> {
        let slots = self.slots.lock();
        let entry = slots.iter().flatten().find(|entry| entry.key() == token).ok_or(ENOENT)?;
        if entry.needs_cleanup || entry.closed || entry.quarantined { return Err(EBUSY); }
    ...
    pub(crate) fn cleanup(&self, token: Token) -> Result {
        ...
        for _ in 0..500 {
            {
                let mut slots = self.slots.lock();
                let slot = slots.iter_mut().find(|slot| slot.as_ref().is_some_and(|entry| entry.key() == token)).ok_or(ENOENT)?;
                let entry = slot.as_ref().unwrap();
                if entry.quarantined { return Err(errno(-71)); }
