...
use super::{application_rpc::{Exchange, Token}, smp_resource::OsToken};
...
impl Remote {
    ...
    pub(crate) fn reserve(&self, pid: i32) -> Result<Token> {
        ...
        if slots.iter().flatten().any(|entry| entry.pid() == pid) { return Err(EINVAL); }
    ...
    pub(crate) fn close(&self, token: Token) {
        ...
        if let Some(slot) = slots.iter_mut().find(|slot|
            ...
            if entry.reserved() || entry.result().is_some() { *slot = None; }
            else { entry.abandon(); }
        }
    }
    ...
    pub(crate) fn cleanup(&self, token: Token) -> Result {
        {
            let mut slots = self.slots.lock();
            let entry = slots.iter_mut().flatten().find(|entry| entry.token() == token).ok_or(ENOENT)?;
        ...
        for _ in 0..500 {
            {
                let mut slots = self.slots.lock();
                let slot = slots.iter_mut().find(|slot|
                    slot.as_ref().is_some_and(|entry| entry.token() == token)).ok_or(ENOENT)?;
