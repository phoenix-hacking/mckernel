...
/// An exclusive host claim on a live guest response. No Rust reference to the
/// guest's mutable byte object is created; shared state uses aligned atomics.
...
impl<'a> Response<'a> {
    ...
    pub(crate) fn prepare(self, servicing_tid: i32, value: i64) -> Result<Completion<'a>, i32> {
        if servicing_tid <= 0 {
            return Err(-22);
        }
        ...
        let status = unsafe { AtomicU64::from_ptr(self.address.add(8).cast()) };
        let state = unsafe { AtomicU64::from_ptr(self.address.add(16).cast()) };
        ...
        unsafe {
            ptr::write_volatile(self.address.add(4).cast::<i32>(), servicing_tid);
            ptr::write_volatile(self.address.add(24).cast::<i64>(), value);
        }
...
impl Completion<'_> {
    ...
    pub(crate) fn publish(
        ...
        unsafe {
            AtomicU64::from_ptr(response.address.add(8).cast()).store(1, Ordering::Release);
        }
        Ok(())
    }
}
