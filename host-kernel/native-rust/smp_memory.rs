...
fn reply_vdso(
    ...
    for page in
        Iterator::chain(response.text_physical.iter(), response.data_physical.iter()).copied()
    {
        if page == 0 {
            continue;
        }
...
pub(super) fn application_topology(
    owner: super::smp_resource::OsToken,
    command: u32,
) -> Result<isize> {
    let published = PUBLISHED.load(Ordering::Acquire);
    ...
    let image = context.images[owner.slot() as usize]
        .as_ref()
        .ok_or(EINVAL)?;
    if image.owner != owner {
...
pub(super) fn application_connection(
    owner: super::smp_resource::OsToken,
    pid: i32,
) -> Result<Application> {
    let published = PUBLISHED.load(Ordering::Acquire);
