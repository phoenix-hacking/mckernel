...
use core::{pin::Pin, ptr::{self, NonNull}, sync::atomic::{AtomicPtr, Ordering}};
...
impl ProcessId {
    fn current() -> Result<Self> {
        ...
        let pid = unsafe { bindings::get_task_pid(bindings::get_current(), bindings::pid_type_PIDTYPE_TGID) };
        Ok(Self(NonNull::new(pid).ok_or(ESRCH)?))
    }
    fn same(&self, other: &Self) -> bool { self.0 == other.0 }
}
...
impl Registry {
    pub(super) fn new() -> Result<Self> {
        ...
        PUBLISHED.compare_exchange(ptr::null_mut(), ptr::from_ref(&*table).cast_mut(), Ordering::AcqRel, Ordering::Acquire)
            .map_err(|_| EBUSY)?;
        Ok(Self(table))
    }
}
    ...
    fn acquire(slot: u32, generation: u64, pid: ProcessId) -> Result<Self> {
        ...
        for entry in &mut *entries {
            if entry.process.matches(slot, generation, &pid) {
                entry.files = entry.files.checked_add(1).ok_or(EOVERFLOW)?;
        ...
        entries.push(Entry { process: process.clone(), files: 1 }, GFP_KERNEL)?;
        Ok(Self(process))
    }
}
...
impl Drop for Binding {
    fn drop(&mut self) {
        let removed = {
            let mut entries = table().lock();
            let index = entries.iter().position(|entry| Arc::ptr_eq(&entry.process, &self.0))
                .expect("native process binding lost its registry entry");
            assert!(entries[index].files != 0);
            entries[index].files -= 1;
            if entries[index].files == 0 { Some(entries.swap_remove(index)) } else { None }
        };
...
impl Context {
    pub(super) fn new(slot: u32, generation: u64) -> Result<Self> {
        Ok(Self { slot, generation, bindings: Box::pin_init(new_mutex!(Vec::new()), GFP_KERNEL)? })
    }
    ...
    fn process(&self) -> Result<Arc<Process>> {
        ...
        for binding in &*bindings {
            if binding.0.pid.same(&pid) { return Ok(binding.0.clone()); }
        }
    ...
    pub(super) fn close_executable(&self) -> Result<isize> {
        ...
        let process = {
            ...
            entries.iter().find(|entry| entry.process.matches(self.slot, self.generation, &pid))
        ...
        Ok(if process.is_some_and(|process| process.replace(None)) { 0 } else { 22 })
    }
}
