...
impl Page {
    ...
    fn busy(&self) -> &AtomicI32 {
        unsafe { AtomicI32::from_ptr(self.pointer().add(1052).cast()) }
    }
}
...
fn layout_matches_the_unchanged_legacy_c_header() {
    ...
    assert_eq!(
        expected,
        [
            size_of::<wire::SetupRequest>(),
            align_of::<wire::SetupRequest>(),
            offset_of!(wire::SetupRequest, error),
            offset_of!(wire::SetupRequest, physical),
            offset_of!(wire::SetupRequest, bytes),
            offset_of!(wire::SetupRequest, busy)
        ]
    );
}
...
fn malformed_fields_alignment_and_errno_do_not_publish_success() {
    ...
    for address in [0, 1, 4095, 4097, u64::MAX] {
        unsafe {
            p.add(8).cast::<u64>().write(address);
        }
        assert_eq!(unsafe { wire::read_setup(p) }, Err(-22));
    }
    ...
    assert_eq!(
        unsafe { wire::complete_setup(std::ptr::null_mut(), 0) },
        Err(-22)
    );
...
fn completion_publishes_owners_and_error_before_peer_can_reuse_request() {
    ...
    let peer = page.clone();
    let owners = published.clone();
    let host = thread::spawn(move || {
        for iteration in 1..=4096 {
            while peer.busy().load(Ordering::Acquire) != 1 {
                thread::yield_now();
            }
            assert_eq!(
                unsafe { wire::read_setup(peer.pointer()) },
                Ok((iteration as u64 * 4096, 4096))
            );
            owners.store(iteration, Ordering::Relaxed);
            unsafe {
                wire::complete_setup(peer.pointer(), if iteration % 2 == 0 { 0 } else { -12 })
            }
            .unwrap();
        }
    });
    for iteration in 1..=4096 {
        let p = page.pointer();
        unsafe {
            p.cast::<i32>().write(-115);
            p.add(8).cast::<u64>().write(iteration as u64 * 4096);
            p.add(16).cast::<i64>().write(4096);
        }
        page.busy().store(1, Ordering::Release);
        while page.busy().load(Ordering::Acquire) != 0 {
            thread::yield_now();
        }
        assert_eq!(published.load(Ordering::Relaxed), iteration);
        assert_eq!(
            unsafe { p.cast::<i32>().read() },
            if iteration % 2 == 0 { 0 } else { -12 }
        );
        // Reuse immediately; the responder must perform no later request access.
        unsafe {
            p.cast::<i32>().write(0x55555555);
        }
    }
    host.join().unwrap();
