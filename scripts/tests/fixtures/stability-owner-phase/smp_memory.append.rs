// VERIFICATION OVERLAY ONLY. Entered directly by the pinned SMP miscdevice file.
pub(super) fn verification_phase_ioctl(argument: usize, compat: bool) -> Result<isize> {
    ...
    if request.phase == phase::SELECT_BLOCKED {
        let key = started.verification_select_read16((request.pid != 0).then_some(request.pid))?;
        if key.os != request.os || key.generation != request.generation {
            return Err(EIO);
        }
        request.bind();
    }
    let result = started.verification_phase_command(request.phase);
    request.consume();
    let code = result.as_ref().err().map_or(0, |error| error.to_errno());
    phase::output(&mut bytes, result.as_ref().copied().unwrap_or(0), code);
    drop(_permit);
