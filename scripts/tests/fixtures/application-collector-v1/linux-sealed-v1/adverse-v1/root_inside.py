#!/usr/bin/env python3
"""Actual root checks followed by one bounded build or four adverse collections."""
...
def main():
    ...
    try:
        ...
        if manifest['mode']=='run': expected.update(['build-record.json','fixture']+['linux-collector-adverse-'+case for case in p.CASES])
        ...
        if manifest['mode']=='build':
            ...
            for case in p.CASES:
                result['cases'].append(runner.run_case(case,HERE/('linux-collector-adverse-'+case),HERE/'fixture',ROOT/case,supervisor))
        verify(); result['status']='PASS_ADVERSE_'+manifest['mode'].upper()+'_INFRASTRUCTURE_ONLY'
