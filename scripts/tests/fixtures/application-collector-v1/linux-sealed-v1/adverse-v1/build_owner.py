...
def flags(selector):
    ...
    return FLAGS + ['-DM02_ADVERSE_TEST_ONLY=1','-DM02_ADVERSE_CASE='+str(selector)]
...
def compile_inside(out,supervisor):
    ...
    try:
        ...
        for name,raw in [('adverse-collector.c',generated),('inject.h',p.read(HERE/'inject.h')),('collector.patch',patch),('source.collector.c',source),('generated.diff',p.generated_diff(source,generated))]:
        ...
        units.update({case:out/'source/linux-sealed-v1/adverse-collector.c' for case in p.CASES})
        ...
        executables = {'fixture':['fixture']}; executables.update({'linux-collector-adverse-'+case:['request','sha256',case] for case in p.CASES})
        ...
        record['status'] = 'PASS_ADVERSE_BUILD_ONLY'
