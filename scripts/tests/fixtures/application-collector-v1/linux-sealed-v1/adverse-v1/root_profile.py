...
def verify_build(path):
    ...
    p.require(record['status']=='PASS_ADVERSE_BUILD_ONLY' and record['application_acceptance'] is False and
    ...
    expected.update(name+suffix for name in ['fixture']+['linux-collector-adverse-'+case for case in p.CASES] for suffix in ('','.map'))
    ...
    for name,data in [('adverse-collector.c',generated),('inject.h',p.read(HERE/'inject.h')),
    ...
    executables=['fixture']+['linux-collector-adverse-'+case for case in p.CASES]
    ...
    for name in units:
        src=prefix/'source/request.c' if name=='request' else prefix/'source/linux-sealed-v1'/('adverse-collector.c' if name in p.CASES else name+'.c')
    ...
    for name in executables:
        objects=['fixture'] if name=='fixture' else ['request','sha256',name[len('linux-collector-adverse-'):]]
    ...
    p.require(p.read(root/'source/linux-sealed-v1/adverse-collector.c')==generated,'generated compiler source')
...
def create(packet,profile,mode,build_record,nonce):
    ...
    manifest={'schema_version':1,'kind':'linux-sealed-adverse-root-inputs','mode':mode,'profile_nonce':nonce,
    ...
    if mode=='run':
        ...
        for name in ['fixture']+['linux-collector-adverse-'+case for case in p.CASES]: sources[name]=build_record.parent/name
    for name,source in sources.items():
        raw=p.read(source); bits=0o755 if name=='fixture' or name.startswith('linux-collector-adverse-') else 0o644
