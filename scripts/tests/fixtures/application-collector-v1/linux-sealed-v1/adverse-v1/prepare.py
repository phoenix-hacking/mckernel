...
EXPECTED_PACKET = 'b9d61a1f74a63d84ffd5dcbdccabdcc950fe5264dc4fbc3c1ac4948b4d883306'
PINS = {
...
def module(path, expected=None):
    ...
    spec = importlib.util.spec_from_file_location('adverse_' + path.stem, path)
...
def generate(source):
    ...
    if source.count(b"m02_adverse_child_boundary") or source.count(b"m02_adverse_completed_wait_hook"): raise ValueError("injection already present")
...
def main():
    ...
    if record_packet.get("schema_version") != 1 or record_packet.get("kind") != "linux-sealed-collector-adverse-v1": raise ValueError("packet kind")
    ...
    if generated.count(b'#include "inject.h"') != 1 or generated.count(b"m02_adverse_child_boundary(setup_fd, words);") != 1: raise ValueError("generated hook count")
    if generated.count(b"m02_adverse_completed_wait_hook") != 1: raise ValueError("generated wait hook count")
    ...
    record = {"schema_version": 2, "kind": "linux-sealed-collector-adverse-preparation", "status": "PREPARED_NOT_EXECUTED", "application_acceptance": False, "backend_enabled": False, "source": {"path": str(SOURCE), "sha256": digest(source)}, "generated": {"path": str(out / "adverse-collector.c"), "sha256": digest(generated), "diff_sha256": digest(diff), "include_count": generated.count(b'#include "inject.h"'), "guard_count": generated.count(b"M02_ADVERSE_TEST_ONLY"), "hook_call_count": generated.count(b"m02_adverse_completed_wait_hook")}, "compiler_supplied": {"M02_ADVERSE_TEST_ONLY": 1, "M02_ADVERSE_CASE": "per-case: 0,1,2,3"}, "patch_sha256": digest(patch_bytes), "header_sha256": digest((HERE / "inject.h").read_bytes()), "cases": CASES, "runtime_root": str(out), "generated_diff_membership": ["inject.h", "m02_adverse_child_boundary", "m02_adverse_completed_wait_hook"]}
