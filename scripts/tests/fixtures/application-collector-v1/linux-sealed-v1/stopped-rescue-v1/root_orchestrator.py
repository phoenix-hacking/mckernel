...
def execute(packet,root,mode,build_record):
    ...
    p.require(root.is_absolute() and root.parent==owner.WORK/'scratch' and root.parent.resolve(strict=True)==root.parent and
              re.fullmatch(r'stability-linux-stopped-rescue-(build|run)-[0-9]{8}-[1-9][0-9]{0,5}',root.name) and not root.exists() and not root.is_symlink(),'fresh dedicated scratch attempt')
    ...
    try:
        ...
        p.require(inner['status']=='PASS_STOPPED_RESCUE_'+mode.upper()+'_INFRASTRUCTURE_ONLY' and inner['profile_nonce']==nonce and inner['application_acceptance'] is False and inner['backend_enabled'] is False,'actual inner result')
        ...
        if commands is not None and submitted:
            try: result['cleanup']=owner.recover_cleanup(commands,config,'stopped-rescue-final',journal=lambda state:owner.recovery_journal(root,'stopped-rescue-final',state))
        ...
        try:
            ...
            if owner.infrastructure_pass_allowed(result,root): result['status']='PASS_STOPPED_RESCUE_'+mode.upper()+'_OWNER_INFRASTRUCTURE_ONLY'
