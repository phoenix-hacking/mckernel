...
/* Dedicated single-threaded rescue protocol.  It is inert unless explicitly
 * selected by the future root runner; no process-group or cached-PID kills. */
static int stopped_rescue_protocol(pid_t collector, pid_t leader, pid_t child)
{
    int status;
    if (waitpid(leader, &status, WUNTRACED | WNOHANG) != leader || status != 4991) return 71;
    if (kill(leader, SIGKILL) != 0) return 72;
    if (waitpid(leader, &status, 0) != leader || status != 9) return 73;
    if (kill(child, SIGKILL) != 0) return 74;
    if (waitpid(child, &status, 0) != child || status != 9) return 75;
    if (kill(collector, SIGTERM) != 0 || kill(collector, SIGCONT) != 0) return 76;
    if (waitpid(collector, &status, 0) != collector || status != 256) return 77;
    errno = 0;
    if (waitpid(-1, &status, WNOHANG) != -1 || errno != ECHILD) return 78;
    return 0;
}
