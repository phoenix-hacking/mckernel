...
use super::super::super::{
    smp_resource::{MemoryExtent, MemoryMap, OsToken},
    sysfs_objects::{AttributeOps, Directory, File},
    sysfs_tree::Tree,
};
use super::{
    errno,
    memory::{Claim, Memory, Span},
    snoop::Snoop,
    wire, AcceptSuccess,
};
use core::{
...
extern "C" {
    fn mckernel_sysfs_snoop_oracle(
        operation: u64,
...
impl Pages {
    fn new() -> Result<Self> {
        ...
        let address = unsafe {
            bindings::get_free_pages_noprof(bindings::GFP_KERNEL | bindings::__GFP_ZERO, ORDER)
        };
        if address == 0 {
            return Err(ENOMEM);
        }
...
impl Drop for Pages {
    fn drop(&mut self) {
        unsafe { bindings::free_pages(self.0 as u64, ORDER) };
    }
}
...
impl Backing {
    fn new() -> Result<Arc<Self>> {
        ...
        map.0[0] = Some(MemoryExtent {
            physical,
            bytes: (112 * PAGE) as u64,
            identity: OsToken(1),
        });
        map.0[1] = Some(MemoryExtent {
            physical: physical + (112 * PAGE) as u64,
            bytes: (16 * PAGE) as u64,
            identity: OsToken(2),
        });
        let mut fixed = Vec::with_capacity(4, GFP_KERNEL)?;
        for index in 0..4 {
            fixed.push(
                Span::new(physical + (index * PAGE) as u64, PAGE)?,
                GFP_KERNEL,
            )?;
        }
        ...
        assert_eq!(
            unsafe { Memory::new(&map, OsToken(3), direct, Vec::new()) }.err(),
            Some(EINVAL)
        );
        Ok(Arc::new(
            Self {
    ...
    fn initialize(&self, kind: wire::Kind, offset: usize) {
        ...
        unsafe {
            ...
            ptr::copy_nonoverlapping(
                b"/sys/probe\0".as_ptr(),
                self.address(offset + layout.path),
                11,
            );
            if kind == wire::Kind::Create {
                ptr::write(self.address(offset).cast::<i32>(), 0o444);
            }
            if kind == wire::Kind::Symlink {
                ptr::write(self.address(offset + 8).cast::<u64>(), 7);
            }
        }
    ...
    fn snoop(&self, operation: u64, offset: usize, bits: i32) -> Result<Snoop> {
        ...
        let instance = if (5..=7).contains(&operation) {
            self.pa(DESCRIPTOR)
        } else {
            self.pa(offset)
        };
...
fn equivalent(backing: &Backing, operation: u64, offset: usize, bits: i32) -> Result {
    ...
    let reference = unsafe {
        mckernel_sysfs_snoop_oracle(
            operation,
            backing.address(offset).cast(),
            bits,
            size,
            expected.as_mut_ptr(),
            PAGE,
...
fn formats(backing: &Backing) -> Result<usize> {
    ...
    for number in [
        ...
        for operation in [1, 2, 3, 4, 8] {
            let offset = if matches!(operation, 1 | 3 | 8) {
                NUMBER + 16
            } else {
                NUMBER
            };
            if matches!(operation, 1 | 3 | 8) {
                backing
                    .word32(offset)
                    .store(number as u32, Ordering::Relaxed);
            } else {
                backing.word64(offset).store(number, Ordering::Relaxed);
            }
            equivalent(backing, operation, offset, 0)?;
            cases += 1;
        }
    }
    for text in [b"\0".as_slice(), b"bounded", b"abc\0ignored"] {
        ...
        equivalent(backing, 5, STRING, (text.len() * 8) as i32)?;
        cases += 1;
    }
    unsafe {
        ptr::write_bytes(backing.address(STRING), b'x', PAGE - 2);
        ptr::write(backing.address(STRING + PAGE - 2), 0);
    }
    equivalent(backing, 5, STRING, ((PAGE - 1) * 8) as i32)?;
    cases += 1;
    for bits in [
        0, 1, 7, 8, 9, 31, 32, 33, 63, 64, 65, 70, 127, 128, 129, 32768,
    ] {
        for byte in [0, 0xff, 0x55] {
            if bits == 32768 && byte == 0x55 {
                continue;
            }
            unsafe { ptr::write_bytes(backing.address(BITMAP), byte, PAGE) };
            for operation in [6, 7] {
                if bits == 32768 && operation == 7 {
                    continue;
                }
                equivalent(backing, operation, BITMAP, bits)?;
                cases += 1;
            }
        }
    }
    ...
    assert_eq!(string.show(&mut buffer(PAGE)?), Err(errno(-75)));
    drop(string);
...
fn mappings(backing: &Backing) -> Result {
    for kind in [
        ...
        assert_eq!(
            backing.memory.claim(kind, backing.pa(REQUEST)).err(),
            Some(EBUSY)
        );
        assert_eq!(
            backing.memory.claim(kind, backing.pa(REQUEST + 8)).err(),
            Some(EBUSY)
        );
        ...
        assert_eq!(
            claim.snoop(backing.pa(112 * PAGE - 4), 8).err(),
            Some(EINVAL)
        );
        ...
        let handle = if matches!(kind, wire::Kind::Mkdir | wire::Kind::Lookup) {
            Some(7)
        } else {
            None
        };
        ...
        assert_eq!(
            unsafe { ptr::read(backing.address(REQUEST + kind.layout().error).cast::<i32>()) },
            0
        );
        assert_eq!(
            backing.memory.claim(kind, backing.pa(REQUEST)).err(),
    ...
    for operation in [1, 2, 3, 4, 8] {
        ...
        for delta in 1..width {
            assert_eq!(
                Snoop::new(&claim, operation, backing.pa(NUMBER + delta)).err(),
                Some(EINVAL)
            );
        }
    }
    for operation in [0, 9, 1000, u64::MAX] {
        assert_eq!(
            Snoop::new(&claim, operation, backing.pa(NUMBER)).err(),
            Some(EINVAL)
        );
    }
    for (operation, bits) in [(5, 0), (5, 1), (5, -1), (6, -1), (7, 32769)] {
        ...
        assert_eq!(
            Snoop::new(&claim, operation, backing.pa(DESCRIPTOR)).err(),
            Some(EINVAL)
        );
    }
    ...
    assert_eq!(
        claim.snoop(backing.pa(NUMBER + 1), 4)?.number(),
        Err(EINVAL)
    );
    ...
    assert_eq!(
        backing
            .memory
            .claim(wire::Kind::Mkdir, backing.pa(SLOW))
            .err(),
        Some(EBUSY)
    );
    drop(first);
    ...
    claim.complete(Ok(None))?;
    backing
        .claim(wire::Kind::Mkdir, SLOW)?
        .complete(Ok(Some(9)))?;

    let mut held = Vec::with_capacity(66, GFP_KERNEL)?;
    for index in 0..66 {
        held.push(
            backing.claim(wire::Kind::Create, REQUEST + index * PAGE)?,
            GFP_KERNEL,
        )?;
    }
    ...
    assert_eq!(
        backing
            .memory
            .claim(wire::Kind::Create, backing.pa(REQUEST + 66 * PAGE))
            .err(),
        Some(ENOMEM)
    );
    assert_eq!(
        backing
            .busy(wire::Kind::Create, REQUEST + 66 * PAGE)
    ...
    while let Some(claim) = held.pop() {
        claim.complete(Err(ENOMEM))?;
    }
    for index in 0..67 {
        assert_eq!(
            backing
                .busy(wire::Kind::Create, REQUEST + index * PAGE)
                .load(Ordering::Acquire),
            0
        );
    }
    ...
    assert_eq!(
        backing.memory.connect(queue, 4 * PAGE, || Err(EIO)),
        Err(EIO)
    );
    let claim = backing.claim(wire::Kind::Create, REQUEST)?;
    drop(claim.snoop(queue, 4)?);
    claim.complete(Ok(None))?;
    backing.memory.connect(queue, 4 * PAGE, || {
        Ok(AcceptSuccess {
            receive_queue: backing.pa(84 * PAGE),
            accepted_channel_cookie: 1,
        })
    })?;
    for offset in [80 * PAGE, 84 * PAGE] {
        assert_eq!(
            backing
                .memory
                .claim(wire::Kind::Create, backing.pa(offset))
                .err(),
            Some(EBUSY)
        );
    }
    assert_eq!(
        backing
            .memory
            .claim(wire::Kind::Create, backing.pa(REQUEST + 1))
            .err(),
        Some(EINVAL)
    );
    Ok(())
    ...
    fn start(job: Job) -> Result<Self> {
        ...
        let task = unsafe {
            bindings::kthread_create_on_node(
                ...
                entry.cast(),
                -1,
                kernel::c_str!("mck-snoop-check").as_char_ptr(),
            )
        };
        if (-4095..0).contains(&(task as isize)) {
            unsafe { drop(Box::from_raw(entry)) };
            return Err(errno(task as isize as i32));
        }
        assert!(!task.is_null());
...
impl AttributeOps for Status {
    fn show(&self, output: &mut [u8]) -> Result<usize> {
        let text = CString::try_from_fmt(fmt!(
            "{} {} {}\n",
            self.0.phase.load(Ordering::Acquire) as u8,
            self.0.reclaimed.load(Ordering::Acquire) as u8,
            self.0.writes.load(Ordering::Acquire)
        ))?;
...
impl AttributeOps for Control {
    fn store(&self, input: &[u8]) -> Result<usize> {
        ...
        if self.writer {
            self.stats.writing.store(enabled, Ordering::Release);
        } else {
            self.stats.gate.store(enabled, Ordering::Release);
        }
        Ok(input.len())
    }
}
...
impl AttributeOps for Remove {
    fn store(&self, input: &[u8]) -> Result<usize> {
        ...
        assert_eq!(
            self.backing
                .memory
                .claim(wire::Kind::Mkdir, self.backing.pa(SLOW))
                .err(),
            Some(EBUSY)
        );
...
impl kernel::Module for Verifier {
    fn init(_module: &'static ThisModule) -> Result<Self> {
        ...
        backing
            .word32(NUMBER + 16)
            .store(INITIAL as u32, Ordering::Relaxed);
        unsafe {
        ...
        let tree = Arc::pin_init(
            new_mutex!(Tree::new(Directory::new(
                Some(&root),
                kernel::c_str!("sys")
            )?)?),
            GFP_KERNEL,
        ...
        for (operation, path, offset, bits) in [
            (1, b"/sys/d32".as_slice(), NUMBER + 16, 0),
            (2, b"/sys/d64", NUMBER, 0),
            (3, b"/sys/u32", NUMBER + 16, 0),
            (4, b"/sys/u64", NUMBER, 0),
            (5, b"/sys/s", STRING, 14 * 8),
            (6, b"/sys/pbl", BITMAP, 70),
            (7, b"/sys/pb", BITMAP, 70),
            (8, b"/sys/u32K", NUMBER + 16, 0),
        ] {
            tree.lock()
        ...
        Ok(Self {
            ...
            status: Some(status),
            gate: Some(gate),
            writer_control: Some(writer_control),
            writer: Some(writer),
            stats,
            _backing: backing,
            _root: root,
