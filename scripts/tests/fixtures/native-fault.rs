...
fn ordinary_authorized_u32_load() {
    for input in [0, 1, 0x12345678, u32::MAX] {
        ...
        assert_eq!(unsafe { native_user_read_u32_checked(&input, &mut output) }, 0);
...
fn exact_kernel_fault_dispatch_preserves_other_state() {
    for (fault, recovery) in [
        ...
        for trap in [13, 14] {
            ...
            assert_eq!(unsafe { native_fault::native_fault_fixup(&mut regs, trap) }, 1);
            assert_eq!(regs.gpr.rip, recovery);
            assert_eq!(regs.gpr.rax as i64, -14);
            assert_eq!((regs.gpr.cs, regs.gpr.rsp, regs.gpr.rdi, regs.gpr.rflags),
...
fn unrelated_traps_pcs_and_user_contexts_are_unchanged() {
    ...
    for (cs, pc, trap) in [(0x33, fault, 14), (0x20, fault + 1, 14),
                            (0x20, 0, 13), (0x20, fault, 6), (0x20, fault, 0)] {
        let mut regs = unsafe { MaybeUninit::<X86UserContext>::zeroed().assume_init() };
        regs.gpr.cs = cs;
        regs.gpr.rip = pc;
        regs.gpr.rax = 99;
        assert_eq!(unsafe { native_fault::native_fault_fixup(&mut regs, trap) }, 0);
        assert_eq!((regs.gpr.cs, regs.gpr.rip, regs.gpr.rax), (cs, pc, 99));
    }
    assert_eq!(unsafe { native_fault::native_fault_fixup(core::ptr::null_mut(), 14) }, 0);
}
