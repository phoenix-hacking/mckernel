...
def specimen(name,phase=1,mode=2):
 ...
 id=dict(os=2,pid=123,generation=9,nonce_low=0x0123456789abcdef,nonce_high=0xfedcba9876543210,mode=mode,
         ...
         key_hex=bytes(key).hex() if state!='initial' else bytes(80).hex())
 req=bytearray(256)
 ...
 after=copy.deepcopy(id);after.update(sequence=seq+1,selected=True,held=phase!=1,released=phase in (8,2,3,4,5),last_snapshot=snapshot,timer_seconds=0 if phase==1 else 35,held_ns=0 if phase==1 else 35700000000,barrier_calls=0 if phase==1 else 2,host_hold_calls=0 if phase==1 else 3 if phase==7 else 5,terminal_ns=40100000000 if phase in (2,3) else 0)
...
def ident(d):
 return '{'+','.join('.'+k+'='+ (arr(bytes.fromhex(v)) if k=='key_hex' else cval(v)) for k,v in d.items()).replace('.key_hex=','.key=')+'}'
