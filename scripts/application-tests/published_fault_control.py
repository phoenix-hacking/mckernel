"""STF2 host control coordinator. Collection metadata, never OS acceptance.
...
is rejected before ACK. The accepted-return ACK latches release_possible before
any send attempt; every later normal/emergency/final physical reader must consult
that flag and must never read the original selected response. A pre-ACK True
read-permission flag is necessary only: the stopped capture must separately
prove the original claim is still retained and unreleased before any read. This module does
not perform physical reads and cannot enforce that external caller obligation.
The mode3 AFTER_EIGHT_HELLO phase is mandatory; its controller is not yet wired. No socket, process, guest or QMP is created by this file.
"""
import hashlib
import json
import math
...
MODES = frozenset(("postpublish-notify", "recoverable-backpressure"))
UART_LIMIT = 16384
FRAME_LIMIT = 512
...
_REQ = re.compile(rb"STF2 REQ" + _COMMON + rb"\n")
_FAIL = re.compile(rb"STF2 FAIL" + _COMMON +
...
class FaultControl:
    ...
    def __init__(self, connection, *, nonce, mode, capture_root, capture_phase,
        ...
        if type(nonce) is not str or re.fullmatch(r"[0-9a-f]{32}", nonce) is None or int(nonce, 16) == 0:
        ...
        self.normal_phases = ("PRE_INPUT", "ACCEPTED_RETURN", "POST_RET",
                              "AFTER_EIGHT_HELLO" if mode == "recoverable-backpressure" else "QUIET")
        ...
        self.release_possible = False
        self.release_ack_sequence = self.release_capture_sha256 = None
    ...
    def _parse(self, frame):
        ...
        if kind == "REQ":
            ...
            if self.awaiting_guest_failure:
                ...
                allowed = {"INPUT", "OWNER_ACCEPTED_RETURN", "ACCEPTED_RETURN"}
            elif self.normal_index == 2:
                allowed = {"OWNER_RELEASE_ACCEPTED", "OBSERVE_RET", "POST_RET"}
                allowed |= ({"OWNER_RECOVERY", "NATURAL_COMPLETION"} if self.mode == "recoverable-backpressure"
                            else {"OWNER_TERMINAL"})
            elif self.normal_index == 3:
                allowed = ({"OWNED_CLEANUP", "HELLO_BATCH", "OWNER_AFTER_EIGHT_HELLO", "AFTER_EIGHT_HELLO"}
                           if self.mode == "recoverable-backpressure" else
                           {"QUIET_INTERVAL", "OWNER_TERMINAL_PLUS_FIVE", "QUIET"})
            else:
    ...
    def _ack(self, request, digest, capture_deadline):
        frame = ("STF2 ACK {nonce} {sequence} {ack_phase} {mode} {tgid} {tid} {start_ticks} "
        ...
        if request["kind"] == "REQ" and request["phase"] == "ACCEPTED_RETURN":
            if self.release_possible or request["sequence"] != 2:
                self._reject("duplicate-or-invalid-release-ack")
            # Latch BEFORE the first send attempt, even if it later writes only
            # part of the ACK or fails. Peer receipt/native dispatch is ambiguous.
            self.release_possible = True
            self.release_ack_sequence = request["sequence"]
            self.release_capture_sha256 = digest
            self._event("release-possible-latched", sequence=request["sequence"], sha256=digest,
                        original_response_reads_allowed=False)
            for fd in self.fds.values():
                os.fsync(fd)
    ...
    def _handle(self, request):
        ...
        request["release_possible"] = self.release_possible
        request["original_response_reads_allowed"] = not self.release_possible
        ...
        try:
            ...
            if (type(result.get("sha256")) is not str or
                    re.fullmatch(r"[0-9a-f]{64}", result["sha256"]) is None or int(result["sha256"], 16) == 0 or result.get("continued") is not True):
                raise ValueError("capture digest and literal continued True required")
        except Exception as exc:
            error = type(exc).__name__ + ":" + str(exc)[:256]
            callback_error = error
        finished = time.monotonic()
        capture_deadline_exceeded = finished >= capture_deadline
        if capture_deadline_exceeded and error is None:
            error = "late-capture-callback"
        if error is not None:
            self._failure("capture-not-confirmed", phase=request["phase"], error=error,
                          callback_error=callback_error, capture_deadline_exceeded=capture_deadline_exceeded)
            self.awaiting_guest_failure = request["kind"] == "REQ"
            return self._event("capture-rejected-no-ack", request=dict(request), error=error,
                               callback_result=retained_result, callback_error=callback_error,
                               capture_deadline_exceeded=capture_deadline_exceeded, awaiting_guest_failure=self.awaiting_guest_failure)
    ...
    def _poll(self, wait_seconds):
        ...
        if b"\n" not in self.buffer:
            ...
            # The actual first received bytes start the frame budget before
            # potentially delayed artifact writes; storage cannot extend it.
            if not self.buffer:
    ...
    def report(self):
        return {"schema_version": 1, "scope": "published-hold-host-UART-coordination-only",
                "release_possible": self.release_possible, "release_ack_sequence": self.release_ack_sequence,
                "release_capture_sha256": self.release_capture_sha256,
                "original_response_reads_allowed": not self.release_possible,
