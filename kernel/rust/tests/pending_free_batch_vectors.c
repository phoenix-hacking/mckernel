...
struct world { struct links source,other,batch; int state,owner; struct page p[4]; int cb[32][3],nc,npanic; };
...
static void emit(struct world*w,const char*name,const char*op,int rc,struct world*before){int i;printf("JSON|{\"case\":\"%s\",\"op\":\"%s\",\"rc\":%d,\"panic\":%d,\"before\":",name,op,rc,w->npanic);snapshot(before);printf(",\"after\":");snapshot(w);printf(",\"callbacks\":[");for(i=0;i<w->nc;i++)printf("%s[%d,%d,%d]",i?",":"",w->cb[i][0],w->cb[i][1],w->cb[i][2]);printf("]}\n");if(rc<0){int panic=w->npanic;if(strcmp(op,"begin-body")==0)w->npanic=before->npanic;assert(!memcmp(before,w,sizeof(*w)));w->npanic=panic;}}
static struct world before(struct world*w){memset(w->cb,0,sizeof(w->cb));w->nc=0;w->npanic=0;return *w;}
...
static void begin_case(struct world*w,const char*name){struct world before_world=before(w);int rc=begin(w,1);emit(w,name,"begin",rc,&before_world);}
static void bb(struct world*w,const char*name){struct world before_world=before(w);int rc=begin_body(w,1);emit(w,name,"begin-body",rc,&before_world);}
static void malformed(struct world*w,const char*name,int boundary){if(boundary==0){init(w);setup(w,0,0);w->source.prev=0;}else{init(w);setup(w,2,0);if(boundary==1)w->p[0].list.prev=2;else w->p[1].list.next=2;}struct world before_world=before(w);int rc=detach(w,1);emit(w,name,"detach",rc,&before_world);}
int main(void){struct world w,b,retained;int i,rc;const char*names[]={"empty","one","three-order"};int counts[]={0,1,3};
    ...
    init(&w);setup(&w,1,0);begin_case(&w,"conflicting-begin");
    init(&w);begin_case(&w,"nested-begin-first");begin_case(&w,"nested-begin-second");
    init(&w);setup(&w,1,0);bb(&w,"begin-panic-bridge");
    init(&w);malformed(&w,"malformed-active-empty",0);
    init(&w);malformed(&w,"malformed-boundary-prev",1);
    init(&w);malformed(&w,"malformed-boundary-next",2);
