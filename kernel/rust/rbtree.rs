...
#[inline(always)]
unsafe fn rb_parent(rb: *const RbNode) -> *mut RbNode {
    ((*rb).__rb_parent_color & !3) as *mut RbNode
}

#[inline(always)]
unsafe fn rb_color(rb: *const RbNode) -> CULong {
    (*rb).__rb_parent_color & 1
}

#[inline(always)]
unsafe fn rb_is_red(rb: *const RbNode) -> bool {
    rb_color(rb) == RB_RED
}

#[inline(always)]
unsafe fn rb_is_black(rb: *const RbNode) -> bool {
...
#[inline(always)]
unsafe fn rb_set_parent(rb: *mut RbNode, p: *mut RbNode) {
...
unsafe fn rb_insert(node: *mut RbNode, root: *mut RbRoot, augment_rotate: RbRotate) {
    ...
    loop {
        if parent.is_null() {
            rb_set_parent_color(node, null_mut(), RB_BLACK);
            break;
...
pub unsafe extern "C" fn rb_erase(node: *mut RbNode, root: *mut RbRoot) {
    let rebalance = rb_erase_augmented(node, root, &DUMMY_CALLBACKS);
